...
$(function(){
    ...
    bootstrapify();
    pagify(".pagify .content", "h3");
    blockify(".blockify .content", "h3");
...
function blockify(_context, _marker) {
    ...
    chunkify(blocksObj, _context, _marker);

    // Load the lesson/page navigation template
    $.get("/templates/blockify.tpl.html", function(data){

        // Inject data in the template that was loaded
        var tpl = Handlebars.compile(data);
        var pageHTML = $(tpl(blocksObj));
...
function pagify(_context, _marker) {
    ...
    chunkify(pageObj, _context, _marker);
...
function chunkify(_object, _context, _marker) {
